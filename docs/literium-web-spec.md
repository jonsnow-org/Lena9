...
**Why this document exists.** A previous attempt to rewrite Literium as a native Kotlin/Flutter app failed because it silently omitted large parts of the product — notifications, follow lists, tweets, the article editor, the entire admin panel — producing an app that looked and behaved like a different, much smaller product. An earlier production "native" app was in fact a **WebView wrapper** around the live website (see `archive/flutter-webview-app/flutter_app/README.md`), built specifically because an even earlier full-native rewrite could not keep pace with the real site's functionality; that WebView wrapper has since been retired in favor of the real native Kotlin/Compose rewrite under `android-native/`, and its pipeline archived rather than deleted. This document exists to prevent that failure from repeating: it errs heavily on the side of over-specifying rather than omitting anything as "obvious."
...
### 13.6 A prior "native" app was a WebView wrapper, built specifically because an earlier full-native rewrite failed — now retired
`archive/flutter-webview-app/flutter_app/README.md` documents, in the codebase's own words, that an earlier attempt at a full native (Flutter) rewrite is exactly what motivated switching to a WebView-wrapper-only architecture — because that native attempt could not keep pace with the real site's functionality. This was strong, first-party corroborating evidence for the concern that motivated commissioning this entire specification document, and should be treated as a cautionary precedent, not merely background trivia: whatever caused that earlier native attempt to drift out of parity (most likely: treating the web app's true breadth as smaller than it is, and building screen-by-screen from assumptions/memory instead of from an exhaustive inventory like this one) is the exact failure mode this document — and the resulting `android-native/` Kotlin/Compose rewrite — exists to prevent from recurring. The WebView wrapper itself has since been retired (its CI pipeline moved to `archive/flutter-webview-app/`, disabled but kept for reference) now that the Kotlin rewrite is the real native app.
