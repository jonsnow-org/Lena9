...
أجب باحترافية ووضوح وأسلوب عربي فصيح ومهذب، وباختصار مناسب للقراءة على الهاتف.`;
  ...
  // ============ محادثة Claude (Anthropic API) ============
  // نسخة مجانية بنموذج اقتصادي (Haiku) بحصة يومية محدودة — بنفس حد
  // المساعد الذكي (Gemini) تماماً — ونسخة كاملة بنموذج أقوى (Sonnet) بعد
  // استنفاد الحصة، تُخصَم كتكلفة ثابتة لكل رسالة من محفظة المستخدم وتُضاف
  ...
  // تكلفة رسالة Sonnet الفعلية على Anthropic لمحادثة عادية (مدخل + مخرج
  // معتدلي الطول) تقارب $0.003–$0.015 حسب طول الرد. $0.01 هنا يبقي هامش
  // ربح معقولاً دون سعر يبدو "مبالغاً فيه" لمستخدم يدفع من محفظته مباشرة
  // (كخصم مصغّر لكل رسالة، لا اشتراك) — أعلى قليلاً من نظائره في تطبيقات
  // الدردشة المدفوعة بالرسالة (عادة كسور السنت إلى سنت واحد). عدّل الرقم
  // هنا وحده إن أردت قيمة أخرى — لا مكان ثانٍ يحتاج تعديلاً.
  const CLAUDE_PAID_MESSAGE_COST = 0.01;
  ...
  function consumeClaudeFreeQuota(uid: string): { allowed: boolean; remaining: number } {
    const now = Date.now();
    let record = claudeChatQuotas.get(uid);
    if (!record || now - record.lastResetTime >= 24 * 60 * 60 * 1000) {
      record = { usedToday: 0, lastResetTime: now };
      claudeChatQuotas.set(uid, record);
    }
    if (record.usedToday >= CLAUDE_FREE_DAILY_LIMIT) {
      return { allowed: false, remaining: 0 };
    }
    record.usedToday += 1;
    return { allowed: true, remaining: Math.max(0, CLAUDE_FREE_DAILY_LIMIT - record.usedToday) };
  }

  async function callClaudeApi(model: string, systemPrompt: string, userMessage: string): Promise<string> {
    ...
    if (!response.ok) {
      ...
      throw new Error(`ANTHROPIC_HTTP_${response.status}: ${errBody.slice(0, 300)}`);
    }

    const data: any = await response.json();
    const textPart = Array.isArray(data.content) ? data.content.find((p: any) => p?.type === 'text') : null;
    return textPart?.text || '';
  }

  app.post('/api/ai/claude-chat', async (req, res) => {
    if (!isAdminConfigured()) {
      return res.status(503).json({ error: 'not_configured', message: 'الخدمة غير مهيأة على الخادم حالياً.' });
    }
    try {
      // نفس منطق توليد الصور تماماً: الهوية تُشتق من توكن Firebase الحقيقي
      // فقط، لا من أي معرّف يرسله العميل — لأن هذا المسار يمس رصيداً مالياً
      // حقيقياً عند تجاوز الحصة المجانية.
      const { uid, email } = await verifyRequestAuth(req.headers.authorization);
      const { message } = req.body;

      if (!message || typeof message !== 'string' || !message.trim()) {
        return res.status(400).json({ error: 'invalid_message', message: 'يرجى كتابة رسالة.' });
      }
      ...
      let userSnap = await userDocRef.get();
      if (!userSnap.exists) {
        await userDocRef.set({ createdAt: new Date().toISOString(), isGuestTracker: true });
        userSnap = await userDocRef.get();
      }
      const userData = userSnap.data() || {};

      const systemPrompt = `أنت Claude من Anthropic، مساعد ذكاء اصطناعي متكامل مدمج داخل منصة "ليتيريوم" (LITERIUM) الأدبية والثقافية العربية.
ساعد المستخدم في أي موضوع يطلبه — كتابة، بحث، برمجة، نصائح، محادثة عامة — بأسلوب واضح ومهني ومختصر يناسب القراءة على الهاتف.`;

      let usedModel: 'haiku' | 'sonnet';
      let shouldCharge = false;
      let remainingFree = 0;
      ...
      if (isOwner) {
        ...
        const quota = consumeClaudeFreeQuota(uid);
        if (quota.allowed) {
          usedModel = 'haiku';
          remainingFree = quota.remaining;
        } else {
          const currentBalance = Number(userData.availableBalance ?? userData.walletBalance ?? 0);
          if (currentBalance < CLAUDE_PAID_MESSAGE_COST) {
            return res.status(402).json({
              error: 'insufficient_balance',
              message: `استنفدت رسائلك المجانية اليوم (${CLAUDE_FREE_DAILY_LIMIT}/${CLAUDE_FREE_DAILY_LIMIT}). رصيدك الحالي ($${currentBalance.toFixed(2)}) غير كافٍ لمتابعة المحادثة بالنسخة الكاملة (تكلفة الرسالة $${CLAUDE_PAID_MESSAGE_COST.toFixed(2)}). يرجى شحن المحفظة للمتابعة.`,
              requiredAmount: CLAUDE_PAID_MESSAGE_COST,
              currentBalance,
      ...
      const modelId = usedModel === 'haiku' ? 'claude-haiku-4-5-20251001' : 'claude-sonnet-5';
      const reply = await callClaudeApi(modelId, systemPrompt, message.trim());
      ...
      if (shouldCharge) {
        try {
          const batch = db.batch();
          batch.update(userDocRef, {
            walletBalance: FieldValue.increment(-CLAUDE_PAID_MESSAGE_COST),
            availableBalance: FieldValue.increment(-CLAUDE_PAID_MESSAGE_COST)
          });
          ...
          if (!ownerQuery.empty) {
            ...
            batch.update(ownerDocRef, {
              walletBalance: FieldValue.increment(CLAUDE_PAID_MESSAGE_COST),
              availableBalance: FieldValue.increment(CLAUDE_PAID_MESSAGE_COST),
              lifetimeEarnings: FieldValue.increment(CLAUDE_PAID_MESSAGE_COST),
              totalEarnings: FieldValue.increment(CLAUDE_PAID_MESSAGE_COST)
            });

            const earningRef = db.collection('earnings').doc();
            batch.set(earningRef, {
              userId: ownerDocRef.id,
              amount: CLAUDE_PAID_MESSAGE_COST,
              type: 'bonus',
              source: `رسالة محادثة Claude من المستخدم (${normalizedEmail || uid})`,
              createdAt: new Date().toISOString(),
              status: 'credited'
            });
          }
          ...
          console.error('Failed to deduct Claude chat cost or credit owner in Firestore:', dbErr);
        }
      }
      ...
      res.json({
        ...
        model: usedModel,
        charged: shouldCharge,
        cost: shouldCharge ? CLAUDE_PAID_MESSAGE_COST : 0,
        remainingFreeUses: usedModel === 'haiku' ? remainingFree : (isOwner ? 9999 : 0),
        newBalance: finalUserBalance
      });
    } catch (err: any) {
      if (err?.message === 'missing_auth_token') {
        return res.status(401).json({
          error: 'auth_required',
          message: 'يتطلب التحدث مع Claude تسجيل الدخول أولاً.'
        });
      }

      const msg = String(err?.message || err || '');
      console.error('Claude Chat Error:', msg, err);

      if (msg === 'ANTHROPIC_API_KEY_MISSING') {
        return res.status(503).json({
          error: 'not_configured',
          message: 'ميزة التحدث مع Claude غير مفعّلة حالياً لأن مفتاح Anthropic API غير مضبوط على الخادم. يرجى إبلاغ إدارة المنصة.'
        });
      }
      if (msg.includes('ANTHROPIC_HTTP_401') || msg.includes('ANTHROPIC_HTTP_403')) {
        return res.status(503).json({ error: 'invalid_api_key', message: 'مفتاح Anthropic API غير صالح. يرجى إبلاغ إدارة المنصة.' });
      }
      if (msg.includes('ANTHROPIC_HTTP_429')) {
        return res.status(429).json({ error: 'rate_limited', message: 'تم تجاوز حصة الاستخدام على مستوى المنصة. يرجى المحاولة بعد قليل.' });
      }

      res.status(500).json({
        error: 'claude_chat_failed',
        message: 'تعذّر الاتصال بـ Claude مؤقتاً. يرجى المحاولة بعد قليل.'
      });
    }
  });
