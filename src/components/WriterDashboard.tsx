    ...
    <div className="space-y-6 animate-fade-in pb-16">
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-5">
            ...
            <div className="p-5 rounded-2xl bg-slate-900 border border-purple-500/20 shadow-sm">
              ...
              <p className="text-xl sm:text-2xl font-black text-white">
                {writer.followersCount.toLocaleString('ar-EG')}
              </p>
              <span className="text-[10px] text-purple-400 font-bold">جمهور متفاعل</span>
            </div>
          ...
          <div className="rounded-3xl bg-slate-900 border border-purple-500/20 p-5 sm:p-6 shadow-sm">
            ...
            <div className="space-y-3">
              {writerArticles.slice(0, 3).map((art) => (
                <div
                  key={art.id}
                  className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 p-4 rounded-2xl border border-purple-500/15 bg-slate-950/60 hover:border-purple-500/40 transition-all"
                >
                  <div className="flex items-center gap-3">
                    <img
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <h4
                          ...
                          className="font-bold text-sm text-white hover:text-purple-300 cursor-pointer"
                        >
                          {art.title}
                        </h4>
                        {art.status === 'draft' && (
                  ...
                  <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
                    ...
                    <button
                      onClick={() => onSelectArticle(art)}
                      className="px-3 py-1.5 rounded-xl bg-purple-600/30 hover:bg-purple-600/50 text-purple-200 text-xs font-bold flex items-center gap-1.5"
                    >
          ...
          <div className="space-y-3">
            {writerArticles.length === 0 ? (
              ...
              writerArticles.map((art) => (
                <div
                  key={art.id}
                  className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 p-4 rounded-2xl border border-purple-500/15 bg-slate-950/60 hover:border-purple-500/40 transition-all"
                >
                  <div className="flex items-center gap-3">
                    <img
                      src={art.featuredImage || 'https://images.unsplash.com/photo-1456513080510-7bf3a84b82f8?w=1200&auto=format&fit=crop&q=80'}
      ...
      {activeSubTab === 'revenue' && (
        <div className="space-y-6">
          ...
          <div className="p-6 rounded-3xl bg-slate-900 border border-purple-500/20 shadow-sm text-center">
            ...
            <button
              onClick={onOpenWallet}
              className="px-6 py-3 rounded-2xl bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-bold text-sm shadow-lg shadow-purple-600/30"
            >
              طلب السحب الفوري (USDT / Bank)
            </button>
          </div>
        </div>
      )}
      ...
      {activeSubTab === 'payouts' && (
        <div className="rounded-3xl bg-slate-900 border border-purple-500/20 p-6 shadow-sm space-y-4">
          ...
          <p className="text-xs text-slate-300">
            الحد الأدنى للسحب هو $20. يتم إيداع الأموال عبر شبكة USDT TRC20 أو الحسابات البنكية المحلية المعتمدة في غضون 24 ساعة.
          </p>
        </div>
      )}
    </div>
