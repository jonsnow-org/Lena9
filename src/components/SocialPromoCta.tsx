...
export const SocialPromoCta: React.FC<SocialPromoCtaProps> = ({ campaign, onClickThrough }) => {
  ...
  return (
    <div className="space-y-2" onClick={(e) => e.stopPropagation()}>
      ...
      {!canVerify && (
        // ⚠️ لا يوجد تحقق تلقائي حقيقي لهذه المنصة فعلياً (X/انستغرام/فيسبوك
        // لا تقدّم واجهة برمجية مجانية لذلك) — لكن النص السابق هنا كان يعلن
        // هذا صراحة للمستخدم ("لا تتيح تقنياً التحقق")، وهذا بالضبط ما يدفعه
        // للنقر بلا أي نية متابعة فعلية بلا أي رادع. لا نستبدله بادّعاء كاذب
        // عن "نظام تحقق صارم" غير موجود فعلياً (كذب على المستخدم بشأن قدرات
        // النظام يعرّض المنصة لمشكلة أكبر من عدم التحقق نفسه)، بل بصياغة
        // صادقة محايدة لا تُفصح عن الثغرة ولا تختلقها.
        <p className="text-center text-[10px] text-slate-400 dark:text-slate-500">
          تم تسجيل زيارتك — أكمل المتابعة على {meta.platformName} فعلياً لدعم صاحب الحساب وتحقيق هدف الحملة.
        </p>
      )}
    </div>
  );
